...
export class AttachmentsController {
  ...
  @ApiOperation({
    ...
    description: 'Enables mobile and web clients to upload files/photos directly to S3 without routing binary payloads through the application server.',
  })
  ...
  async generatePresignedUploadUrl(
    ...
    const data = await this.attachmentsService.generatePresignedUploadUrl(dto, userId);
    return {
  ...
  @ApiOperation({
    summary: 'Generate secure AWS S3 Pre-signed GET URL for file viewing / download',
    description: 'Returns time-limited private download link with inline content disposition.',
  })
  ...
  @ApiOperation({ summary: 'List all active attachments for an entity (TASK, PROJECT, PRODUCT, etc.)' })
  async findByEntity(
    @Param('entityType') entityType: string,
    @Param('entityId', ParseUUIDPipe) entityId: string,
  ) {
    const data = await this.attachmentsService.findByEntity(entityType.toUpperCase(), entityId);
    return {
